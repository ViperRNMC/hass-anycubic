...
class AnycubicPrinter:
    ...
    def _process_mqtt_update_temperature(
        ...
        if action in ['auto', 'query'] and state in ['done', 'success']:
            ...
            if self.parameter and curr_hotbed is not None and curr_nozzle is not None:
                self.parameter.update_current_temps(
                    curr_hotbed,
                    curr_nozzle,
                )
            if self._latest_project and target_hotbed is not None and target_nozzle is not None:
                self._latest_project.update_target_temps(
                    target_hotbed,
                    target_nozzle,
                )

            # Optional chamber keys may be present on newer firmware.
            data.get('curr_chamber_temp')
            data.get('target_chamber_temp')

            # Ignore any additional unknown temperature keys from newer firmware.
            data.force_empty()
            return
        else:
    ...
    def _process_mqtt_update_fan(
        ...
        if action == 'setSpeed' and state == 'done':
            # Confirmation response after setting fan speed; data is None, nothing to store.
            return
        if action in ['auto', 'query'] and state in ['done', 'success']:
            data = payload.get('data', {})

            if data.get('fan_speed_pct') is not None:
                self._fan_speed = int(data['fan_speed_pct'])

            # Optional fan keys on some printers/firmware.
            if data.get('aux_fan_speed_pct') is not None:
                self._aux_fan_speed_pct = int(data['aux_fan_speed_pct'])
            if data.get('box_fan_level') is not None:
                self._box_fan_level = int(data['box_fan_level'])

            # Ignore any additional unknown fan keys from newer firmware.
            data.force_empty()
            return
        else:
    ...
    def _process_mqtt_update_print(
        ...
        if action == 'start' and state == 'printing':
            ...
            if isinstance(data, AnycubicConsumableData):
                data.force_empty()
            return
        elif action == 'start' and state in ['auto_leveling', 'autoLeveling', 'leveling']:
            data = payload.get('data', {})
            self._is_printing = 2
            # Treat auto-leveling as an active pre-print phase.
            self._update_latest_project_with_mqtt_print_status_data(
                project_id,
                AnycubicPrintStatus.Preheating,
                data,
            ...
            if data['settings'].get('aux_fan_speed_pct') is not None:
                self._aux_fan_speed_pct = int(data['settings']['aux_fan_speed_pct'])
            if data['settings'].get('box_fan_level') is not None:
                self._box_fan_level = int(data['settings']['box_fan_level'])
        ...
        elif action in ['start', 'update', 'pause', 'resume', 'stop']:
            # Be tolerant for newer firmware states we don't map yet.
            data = payload.get('data', {})
            if action == 'stop':
                self._is_printing = 1
                mapped_status = AnycubicPrintStatus.Cancelled
            else:
                self._is_printing = 2
                mapped_status = AnycubicPrintStatus.Printing
            self._update_latest_project_with_mqtt_print_status_data(
                project_id,
                mapped_status,
                data,
    ...
    def _process_mqtt_update_multicolorbox(
        ...
        if action == 'getInfo' and state == 'success':
            ...
            data = data_root.get('multi_color_box')
            # Optional metadata on some firmware (does not affect entity state).
            data_root.get('head_tools_model')
    ...
    def secondary_multi_color_box_loaded_slot(self) -> int | None:
        if self.secondary_multi_color_box:
            return self.secondary_multi_color_box.loaded_slot
    ...
    async def change_print_setting_speed_mode(
        ...
        return await self._api_parent._send_order_change_print_settings(
            printer=self,
            print_settings=self._build_print_settings(print_speed_mode=new_speed),
        )
