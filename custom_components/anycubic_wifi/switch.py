...
class AnycubicSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def _publish(self, action: str, data: dict) -> None:
        try:
            mqtt = self.coordinator.mqtt
            topic = mqtt.web_topic(self.definition.get("type"))
            payload = {"type": self.definition.get("type"), "action": action, "data": data}
            mqtt.publish_json(topic, payload)
        except Exception as err:  # pragma: no cover - defensive
            _LOGGER.debug("MQTT publish failed for switch %s: %s", self._key, err)
    ...
    def device_info(self) -> dict:
        ...
        if dt == DEVICE_TYPE_ACE_PRO:
            # attach to the per-box Ace Pro device using the actual box id
            return {"identifiers": {(DOMAIN, f"{entry_id}_ace_pro_box_{self.box_id}")}, **ACE_PRO_DEVICE_BASE}
